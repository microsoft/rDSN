...
Many people asked about what kinds of replication protocol we're using. Currently, our default provider largely follows [PacificA](http://research.microsoft.com/apps/pubs/default.aspx?id=66814). We may have other protocols as framework providers in layer 2 in the future.

### STEP 1. Code generation

Code generation is very similar to what we do for the single-node service development before, except for the following two minor additions.

##### STEP 1.1 Annotate the interface, tell rDSN which methods need to be replicated.

Methods for a service usually contain both read-only and write methods. Replication is necessary for only those write methods. Developers therefore use a separated annotation file (in order to support both Thrift and Google Protoc, as well as further ones) to mark them. Following is the example as in [here](https://github.com/Microsoft/rDSN/blob/master/tutorial/counter.thrift.annotations).
...
write = false
```
##### STEP 1.2 Generate the code with both the IDL definition file and the annotation file as inputs.

This is exactly the same compared to [step 1.2 for single-node development](https://github.com/Microsoft/rDSN/wiki/Tutorial:-Build-A-Single-Node-Counter-Service#step-12), except that we add the **replication** argument for the code generation tool.
...
This remains exactly the [same](https://github.com/Microsoft/rDSN/wiki/Tutorial:-Build-A-Single-Node-Counter-Service#step-2-implement-application-logic).

### STEP 3. Implement application specific replication logic

This is the extra step we need for replication, mostly about writing helper functions for replication so that it can work smoothly and efficiently for the given application. Description about the helper functions can be found [here](https://github.com/Microsoft/rDSN/blob/master/include/dsn/dist/replication/replication_app_base.h#L50), and the implementation for our counter service example is [here](https://github.com/Microsoft/rDSN/tree/master/tutorial/counter.replication).

### STEP 4. Register the implementation, compile and run

We register the new service implementation to rDSN in main (done by code generation), and compile should be ok.
...
It's natural that we sometimes want to test codes by running multiple programs simultaneously. We provide a simple script for occasions like that. In the "tutorial/counter.test/" folder we prepared an example. "test" is for simulator mode and "vtest" is for nativerun mode. They're basically the same thing with different config files.

To use it, copy the .cmd and .ini files into the folder containing .exe file. Remember to set the iteration times and program name in the .cmd. And configure the .ini file if necessary. Then run the .cmd files.
