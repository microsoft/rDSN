...
function kill_app()
{
    ps aux | grep -F "$exe" | grep -F "$cfg" | grep -F "$1@$2" | grep -v grep | awk '{print $2}' | xargs -r kill -9
}
...
{
    ps aux | grep -F "$exe" | grep -F "$cfg" | grep -v grep | awk '{print $2}' | xargs -r kill -9
}
...
function random_kill_replica()
{
    ...
    while [ $core_result == 0 ]; do
        ...
        echo "kill replica $random_replica_id"
...
case $1 in
    ...
    kill_svc)
        if [ $# -eq 2 ]; then
            ps aux | grep -F "$exe" | grep -F "$cfg" | grep -F "$2" | grep -v grep | awk '{print $2}' | xargs -r kill -9
        elif [ $# -eq 3 ]; then
            kill_app "$2" "$3"
        fi
        ;;
    list)
        ps aux | grep -F "$exe" | grep -F "$cfg"
esac
